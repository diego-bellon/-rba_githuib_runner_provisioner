...
function buildUserDataScript(ghtoken, label, runnerVersion) {
    const userData = [
        ...
        `echo "checkSDPStatus(){" >> get-runner-token.sh`,
        ...
        `echo " fi" >> get-runner-token.sh`,
        `echo "}" >> get-runner-token.sh`,
        `echo "checkSDPStatus" >> get-runner-token.sh`,
        `echo "#Getting registration token" >> get-runner-token.sh`,
        `echo "date2_start=\\$(date --date='+0 seconds'  +\\"%Y-%m-%d %H:%M:%S\\")" >> get-runner-token.sh`,
        `echo "date2_finish=\\$(date --date='+5 minutes' +\\"%Y-%m-%d %H:%M:%S\\")" >> get-runner-token.sh`,
        `echo "registration_token=\\"null\\"" >> get-runner-token.sh`,
        `echo "while [[ \\"\\$registration_token\\" == \\"null\\" && \\$date2_start < \\$date2_finish ]]; do" >> get-runner-token.sh`,
        `echo " date2_start=\\$(date --date='+0 seconds'  +\\"%Y-%m-%d %H:%M:%S\\")" >> get-runner-token.sh`,
        ...
        `echo " if [[ \\$response == *\\"Connection reset by peer\\"* ]]; then" >> get-runner-token.sh`,
        `echo "  echo \\"Error: Connection reset by peer. Repitiendo la petición...\\"" >> get-runner-token.sh`,
        `echo "  registration_token=\\"null\\"" >> get-runner-token.sh`,
        `echo " else" >> get-runner-token.sh`,
        ...
        `echo "date3_start=\\$(date --date='+0 seconds'  +\\"%Y-%m-%d %H:%M:%S\\")" >> get-runner-token.sh`,
        `echo "date3_finish=\\$(date --date='+5 minutes' +\\"%Y-%m-%d %H:%M:%S\\")" >> get-runner-token.sh`,
        `echo "while [[ \\$date3_start < \\$date3_finish ]]; do" >> get-runner-token.sh`,
        `echo " date3_start=\\$(date --date='+0 seconds'  +\\"%Y-%m-%d %H:%M:%S\\")" >> get-runner-token.sh`,
        'echo " checkSDPStatus" >> get-runner-token.sh',
        ...
        `echo " if [ \\$? -eq 0 ]; then" >> get-runner-token.sh`,
        `echo "  echo \\"Config ok.\\"" >> get-runner-token.sh`,
        `echo "  break" >> get-runner-token.sh`,
        `echo " else" >> get-runner-token.sh`,
        `echo "  echo \\"Config error. Repeating...\\"" >> get-runner-token.sh`,
        'echo "  sleep 10" >> get-runner-token.sh',
        `echo " fi" >> get-runner-token.sh`,
        `echo "done" >> get-runner-token.sh`,
        `echo "date4_start=\\$(date --date='+0 seconds'  +\\"%Y-%m-%d %H:%M:%S\\")" >> get-runner-token.sh`,
        `echo "date4_finish=\\$(date --date='+5 minutes' +\\"%Y-%m-%d %H:%M:%S\\")" >> get-runner-token.sh`,
        `echo "while [[ \\$date4_start < \\$date4_finish ]]; do" >> get-runner-token.sh`,
        `echo " date4_start=\\$(date --date='+0 seconds'  +\\"%Y-%m-%d %H:%M:%S\\")" >> get-runner-token.sh`,
        'echo " checkSDPStatus" >> get-runner-token.sh',
        `echo " ./run.sh" >> get-runner-token.sh`,
        `echo " if [ \\$? -eq 0 ]; then" >> get-runner-token.sh`,
        `echo "  echo \\"Run ok.\\"" >> get-runner-token.sh`,
        `echo "  break" >> get-runner-token.sh`,
        `echo " else" >> get-runner-token.sh`,
        `echo "  echo \\"Run error. Repeating...\\"" >> get-runner-token.sh`,
        'echo "  sleep 10" >> get-runner-token.sh',
        `echo " fi" >> get-runner-token.sh`,
        `echo "done" >> get-runner-token.sh`,
