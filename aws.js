...
function buildUserDataScript(ghtoken, label, runnerVersion) {
    const userData = [
        ...
        `echo "./config.sh --unattended --url https://github.com/${config.githubContext.owner}/${config.githubContext.repo} --token \\$registration_token --labels ${label} --name self-hosted-runner-${config.generateUniqueLabel()} --replace --path ./actions-runnner" >> get-runner-token.sh`,
